...
void GestorUsuarios::verificarUsuario(const std::string& correo, const std::string& contrasena) {
    ...
    if (!estado) {
        ...
        if (fila) {
            ...
            std::string token = Utilidades::generarToken(); // Usar la función de utilidad
            ...
            std::cout << "Set-Cookie: contrasena=" << contrasena << "; path=/\r\n"; // Considera no guardar contraseñas en cookies, o encriptarlas
            std::cout << "\r\n";
            ...
            std::cout << "Content-type: text/html\r\n\r\n";
            std::cout << "<html><body><h2>Usuario o contraseña incorrectos</h2></body></html>";
        }
...
void GestorUsuarios::registrarUsuario(const std::string& correo, const std::string& contrasena, const std::string& nombre, const std::string& rol, const std::string& idUsuarioAuditor) {
    ...
    if (mysql_query(conexionDB, query.c_str()) == 0) {
        gestorAuditoria.registrarAuditoria(idUsuarioAuditor, "usuarios", "INSERT", "Registró el usuario " + nombre + " con rol " + rol);
        ...
        std::cout << "<html><head><script>";
        std::cout << "alert('❌ Error al registrar. El correo puede estar duplicado o hay otro problema: " << mysql_error(conexionDB) << "');";
...
void GestorUsuarios::listarUsuarios() {
    std::string query = "SELECT id, usuario, correo, rol FROM laboratorioquimica.usuarios"; // No seleccionamos la contraseña
    MYSQL_RES* resultado;
    MYSQL_ROW fila;
    std::cout << "Content-type: text/html\r\n\r\n";
    ...
    if (mysql_query(conexionDB, query.c_str()) == 0) {
        ...
        while ((fila = mysql_fetch_row(resultado)) != nullptr) {
            ...
            std::string rol = fila[3]; // Ahora es fila[3] porque quitamos la contraseña
            ...
            std::cout << "<td>"
                ...
                << "&correo=" << correo
                << "&rol=" << rol
                << "'>" // No pasar la contraseña por URL, no es buena práctica.
...
void GestorUsuarios::actualizarUsuario(const std::string& id, const std::string& nuevoNombre, const std::string& nuevaContrasena, const std::string& idUsuarioAuditor) {
    std::string consulta = "UPDATE usuarios SET usuario='" + nuevoNombre + "', contrasena='" + nuevaContrasena + "' WHERE id='" + id + "'";

    std::cout << "Content-type: text/html\r\n\r\n";
    // std::cout << "<!-- Consulta: " << consulta << " -->"; // Para depuración, comentar en producción

    if (mysql_query(conexionDB, consulta.c_str()) == 0) {
        gestorAuditoria.registrarAuditoria(idUsuarioAuditor, "usuarios", "UPDATE", "Actualizó el usuario " + nuevoNombre + " (ID: " + id + ")");
        std::cout << "<html><head><script>";
        std::cout << "alert('✅ Actualización exitosa');";
        std::cout << "window.location.href = 'http://localhost/PaginaWebLaboratorio/editarPerfil.html';";
        std::cout << "</script></head><body></body></html>";
    }
    else {
        std::string errorMsg = mysql_error(conexionDB);
        // Sanitizar el mensaje de error para JavaScript
...
void GestorUsuarios::eliminarUsuario(const std::string& id, const std::string& idUsuarioAuditor) {
    // Primero, obtener el nombre del usuario antes de eliminarlo para la auditoría
    ...
    if (mysql_query(conexionDB, consulta.c_str()) == 0) {
        std::string descripcion = "Eliminó el usuario " + nombreUsuarioEliminado + " con ID " + id;
